...
class ValueTracker {
    ...
    _emit_update(update) {
        ...
        for (let id of Object.getOwnPropertyNames(this.#watches)) {
            let tmp = this.#watches[id];
            // This can happen if `cb` below causes an unwatch
            if (!tmp)
                continue;
            let [watch, cb] = tmp;
            let watch_update;
    ...
    watch(watch, cb) {
        ...
        // The caller, especially when doing a reconnection
        // may not have fetched the latest cache.
        // Fire the callback asynchronously
        // to be as closed to a real update event as possible.
        setTimeout(() => { cb(this.get_cached(watch)); });
        return id;
    }
